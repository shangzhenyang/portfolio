...
function ListItem({ className, item }: ListItemProps): JSX.Element {
	...
	const tagElements = item.tags.map((tag) => {
		return (
			<span key={tag} className="bg-dark-blue/50 border inline-block whitespace-nowrap m-px px-2.5 py-1 rounded border-solid border-white/10">
				{tag}
			</span>
		);
	});
	return (
		<div>
			...
			{!isChinaSite && item.tags.length > 0 &&
				<div className={classNames(descriptionStyles, "pt-1 pb-4 px-4")}>
					{tagElements}
				</div>
			}
		</div>
